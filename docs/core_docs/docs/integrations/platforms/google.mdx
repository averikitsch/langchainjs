...
The [Postgres Chat Message History](docs/integrations/memory/google_cloudsql_pg.mdx) from the
@langchain/google-cloud-sql-pg provides a way to use the CloudSQL for PostgresSQL  messages and 
provide conversation history for Postgres using the class.
...
```typescript
const historyInstace = await PostgresChatMessageHistory.create(
  engine, 
  "test", 
  "chat_message_table"
);
